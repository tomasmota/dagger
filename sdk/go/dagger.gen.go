...
func (r *Container) WithFiles(path string, files []*File, opts ...ContainerWithFilesOpts) *Container {
...
func (r *Directory) WithFiles(path string, files []*File, opts ...DirectoryWithFilesOpts) *Directory {
	q := r.q.Select("withFiles")
	for i := len(opts) - 1; i >= 0; i-- {
		// `permissions` optional argument
		if !querybuilder.IsZeroValue(opts[i].Permissions) {
